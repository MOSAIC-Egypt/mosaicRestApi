...
router.put("/:email/edit", function (req, res, next) {
  let emailID = req.params.email;
    ...
    User.getUserByEmail(emailID, function (err, user) {
      if (err) return next(err);
      if (!user) {
        let err = new TypedError("User not found", 403, "invalid_field", {
      ...
      bcrypt.genSalt(10, function (err, salt) {
        bcrypt.hash( req.body.password, salt, function (err, hash) {
          ...
          User.findOneAndUpdate({email: email}, req.body, {new: true}, function(err, user) {
            if (err)
              res.send(err);
            res.json(user);
          });
